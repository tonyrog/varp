...
static ERL_NIF_TERM varp_get_clause(ErlNifEnv* env, int argc,
				    const ERL_NIF_TERM argv[]);
static ERL_NIF_TERM varp_get_clause_info(ErlNifEnv* env, int argc,
					  const ERL_NIF_TERM argv[]);
...
typedef struct _literal_t
{
    ...
    struct _plink_t* plist;    // list of 2-clause triggers
} literal_t;
...
typedef struct _varp_t {
    ...
    allocator_t plink_allocator;   // heap storage for plink_t
} varp_t;
...
ErlNifFunc varp_funcs[] = 
{
    ...
    NIF_FUNC( "get_clause_info",     3,  varp_get_clause_info ),
...
static inline literal_t* var_literal(varp_t* vp, variable_t* var)
{
    ...
    if (v == ITRUE) return &var->lit[LIT_POS];
    else if (v == IFALSE) return &var->lit[LIT_NEG];
    else {
	assert(0);
	return NULL;
    }
...
static void log_permanent(varp_t* vp, literal_t* x, literal_t* y, int level)
{
	...
	while(sp != NULL) {
		...
		if (y == NULL) {
		    xt = enif_make_int(vp->msg_env,export_ll(x));
		    bnd = xt;
		}
		else { // if (y->var->flags & VAR_FLAG_ATOM) {
		    yt = enif_make_int(vp->msg_env,export_ll(y));
		    xt = enif_make_int(vp->msg_env,export_ll(x));
			    ...
			    long li, cix_t cix,int level)
{
	...
	lqueue_put(vp, (ivalue==ITRUE) ? neg_l(l) : l);
		   ...
		   long li,cix_t cix,int level)
{
    ...
    if (is_constant(ivalue))
	lqueue_put_ll(vp, (ivalue==ITRUE) ? neg_ll(lp) : lp);
}
...
static cix_t clause_insert(varp_t* vp, clause_t* cp)
{
    int i = vp->cnext++;
...
static void cleanup(varp_t* vp)
{
    ...
    cleanup_allocator(&vp->plink_allocator);
}
			     ...
			     const ERL_NIF_TERM argv[])
{
	...
	goto error;
    if (init_allocator(&vp->plink_allocator, sizeof(plink_t)) < 0)
	goto error;    
				      ...
				      const ERL_NIF_TERM argv[])
{
    ...
    init_variable(vp, var, IUNDEF, vix);
    vp->var_map[vix] = var;
...
static void order_k_degree(varp_t* vp, int k)
{
	...
	for (j = 0; j < (int)cp->size; j++) {
	    ...
	    if (x < 1)
		vp->var_map[-x]->nkey[k] += 1.0f;
	    else if (x > 1)
		vp->var_map[x]->pkey[k] += 1.0f;
	}
    }
}
				  ...
				  const ERL_NIF_TERM argv[])
{
	...
	return enif_make_badarg(env);
    return make_boolean(env, get_vv(vp, var) != IUNDEF);
}
...
void print_lit_array(char* label, lit_t* lit, size_t size)
{
	...
	unsigned k;
	printf("%s=[%d", label, export_l(lit[0]));
	for (k=1; k<size; k++)
	    printf(",%d", export_l(lit[k]));
	printf("]\r\n");
    }
}
...
// eval clause
// return (literal_t*) 0   non conclusive (dead)
...
//	vp->clause_eval_counter[DEAD_CLAUSE]++;
//	return 0;
//    }
	...
	if (cnt < 0) {  // no new watch point found
		...
		put_nq_ll(vp, lp1, ITRUE, wp1, cp->cix, vp->level);
		return lp1;
	    }
	}
	else {  // move watch
	...
	if (cnt < 0) {  // no new watch point found
		...
		put_nq_ll(vp, lp0, ITRUE, wp0, cp->cix, vp->level);
		return lp0;
	    }
	}
	else {  // move watch
...
static int eval_clauses(varp_t* vp, literal_t* lp)
{
	...
	if (!(cp->flags & (CLAUSE_FLAG_CONFLICT|CLAUSE_FLAG_DEAD))) {
	    lp = eval_clause(vp, cp, wlink_index(wl), wlp);
	    if (lp == (literal_t*) 0) {
		if (vp->max_conflicting == 1) {
...
static int eval1(varp_t* vp, literal_t* lp)
{
    if (eval_clauses(vp, lp) < 0)
	return -1;
#ifdef TWO_CLAUSES
    if (eval_2_clauses(vp, lp) < 0)
	return -1;
#endif
    return 0;
}
				     ...
				     lit_t* lit, size_t size)
{
    ...
    if (size == 1) {  // unit
	if (lit[0] == VARP_FALSE(vp))
	    return ATOM(false);
	put_l(vp, lit[0], ITRUE, -1, -1, 0);
	return ATOM(true);
    }
	...
	goto error;
    if ((cix = clause_insert(vp, cp)) == CLAUSE_ERROR)
	goto error;
