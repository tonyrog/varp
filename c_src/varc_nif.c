...
#define ALPHA 2    // temporary clause set
#define BETA  3    // temporary clause set

#define NUM_CSET 4

typedef struct _varp_opt_t {
...
typedef struct _varp_t {
    ...
    int          first_unbound; // index in order_map of
    int          next_unbound;// next
...
{
    unsigned k;
    enif_fprintf(stdout, "%s id=%d:%d,[%ld:%ld] [%d/%s",
...
static void* dmem_alloc(varp_t* vp, size_t size)
{
	...
	size_t memsize = size < 256 ? 256 : 2*size;
	if ((vp->dmem = VARP_ALLOC(memsize)) == NULL)
	    return NULL;
	vp->dmem_size = memsize;
    }
    else if (vp->dmem_size < size) {  // realloc
	size_t memsize = 2*size;
	if ((vp->dmem = VARP_REALLOC(vp->dmem, memsize)) == NULL)
	    return NULL;
	vp->dmem_size = memsize;
    }
    return vp->dmem;
...
static int make_sub_info(varp_t* vp,uint32_t flags,ERL_NIF_TERM* info)
{
    ...
    return enif_make_map_from_arrays(env, sub_info_keys, values, 7, info);
}
...
static void unbind_level(varp_t* vp, int level)
{
	...
	clr_vv(vp, bp);
	if (bp->map_index < vp->next_unbound)
	    vp->next_unbound = bp->map_index;
	DBG_BCP("%sNextvar=%d\r\n", indent(level), vp->next_unbound);
	bp = bp->next;
	nbound++;
    }
...
// resize clause hash table
static int resize_hashtab(varp_t* vp, size_t size)
{
    hlink_t** htab;
    int i;

    if ((htab = VARP_ALLOC(size*sizeof(hlink_t**))) == NULL)
	return -1;
    memset(htab, 0, size*sizeof(hlink_t**));
...
static int insert_hash_link(varp_t* vp, hlink_t* hp)
{
    int hix;
    
    if (vp->hnum+1 >= vp->hsize) {
	size_t hsize1 = vp->hsize*2;
	if (resize_hashtab(vp, hsize1) < 0)
	    return -1;
    }
    vp->hnum++;
...
static int alloc_hashtab(varp_t* vp, size_t size)
{
    if (!(vp->hashtab = VARP_ALLOC(size*sizeof(hlink_t**))))
	return -1;
...
static void cleanup(varp_t* vp)
{
    ...
    // turn off, avoid free all hash links clause_free, they are
    // all deallocated when the allocator is destroyed
    if (vp->opt.hash) {
	VARP_FREE(vp->hashtab);
	vp->hashtab = NULL;
	vp->opt.hash = false;  
...
static void default_varp_opts(varp_opt_t* opt)
{
    opt->qtype = lifo;
    opt->atype = off;
    opt->hash  = false;
    ...
    opt->csize  = DEFAULT_MAP_SIZE;
}

static int parse_varp_opts(ErlNifEnv* env, ERL_NIF_TERM list, varp_opt_t* opt)
{
	...
	else if (elem[0] == ATOM(hash) && (elem[1] == ATOM(true))) {
	    opt->hash = true;
	}
...
static int alloc_symtab(varp_t* vp, size_t size)
{
    void* ptr = VARP_ALLOC(size*sizeof(symbol_t**));
    if (ptr == NULL)
	return -1;
    memset(ptr, 0, size*sizeof(symbol_t**));
    vp->symtab = ptr;
    vp->ssize = size;
    return 0;
}

static int resize_symtab(varp_t* vp, size_t size)
{
    symbol_t** symtab = VARP_ALLOC(size*sizeof(symbol_t**));
    int i;
    if (symtab == NULL)
	return -1;
    memset(symtab, 0, size*sizeof(symbol_t**));
    for (i = 0; i < (int)vp->ssize; i++) {
	symbol_t* sp = vp->symtab[i];
	while(sp != NULL) {
...
static int setup(varp_t* vp, varp_opt_t* opt)
{
    size_t vsize, csize, ssize;
    int i;
    ...
    if (vp->opt.hash) {
	size_t hsize = 1;
	while(hsize < csize) hsize *= 2;
	if (alloc_hashtab(vp, hsize) < 0) 
	    goto error;
    }
			     ...
			     const ERL_NIF_TERM argv[])
{
    ...
    default_varp_opts(&opt);

    if (parse_varp_opts(env, argv[0], &opt) < 0)
...
static int realloc_variables(varp_t* vp, size_t vsize)
{
    ...
    if (!(ptr = VARP_REALLOC(vp->var_map, vsize*sizeof(variable_t*))))
	return -1;
    vp->var_map = ptr;
	
    if (!(ptr = VARP_REALLOC(vp->order_map, vsize*sizeof(int))))
	return -1;
    vp->order_map = ptr;
#ifdef PACKED_VALUE
#ifdef LIT_VALUE
    if (!(ptr = VARP_REALLOC(vp->lit_value,
			     2*PACKED_BYTES(vsize)*sizeof(uint8_t))))
	return -1;
    vp->lit_value = ptr;
    vp->lit_overlay = (uint16_t*) ptr;
#else
    if (!(ptr = VARP_REALLOC(vp->var_value,
			     PACKED_BYTES(vsize)*sizeof(uint8_t))))
	return -1;
    vp->var_value = ptr;
#endif
#endif	
					  ...
					  const ERL_NIF_TERM argv[])
{
    ...
    vp->first_unbound = ui;  // remember the position
	...
	if (!is_marked(vp->var_map[xi])) { // not moved
	    mark_variable(vp, vp->var_map[xi]);     // mark as moved
					 ...
					 const ERL_NIF_TERM argv[])
{
	...
	if (!is_marked(lp->var)) {  // not moved
	    mark_variable(vp, lp->var);  // mark as moved
	    lp->var->map_index = --mi;
	    map[mi] = export_ll(lp);
	}
	list = tail;
				  ...
				  const ERL_NIF_TERM argv[])
{
    ...
    dsize = 1024;
    dlit = (lit_t*) dmem_alloc(vp, dsize*sizeof(lit_t));
    csize = 0;
				  ...
				  const ERL_NIF_TERM argv[])
{
    ...
    if ((cp = get_clause(vp, cix)) == NULL)
	return enif_make_badarg(env);
    // check that clause is in BETA ?
    // otherwise we need to update xref/watch points etc

    dlit = (lit_t*) dmem_alloc(vp, cp->size*sizeof(lit_t));
    csize = 0;
    n = (int) cp->size;
    for (i = 0; i < n; i++) {
	lit_t li = cp->lit[i];
...
static void clause_xref(varp_t* vp, clause_t* cp)
{
    long n = (long)cp->size;
    long p;
    for (p = 0; p < n; p++)
...
static void clause_unxref(varp_t* vp, clause_t* cp)
{
    long n = (long)cp->size;
    long p;
			       ...
			       const ERL_NIF_TERM argv[])
{
    ...
    if (parse_varp_opts(env, argv[1], &opt) < 0)
	return enif_make_badarg(env);
			      ...
			      const ERL_NIF_TERM argv[])
{
    ...
    if (argv[1] == ATOM(edge)) {
	return make_boolean(env, vp->opt.edge);
    }
    if (argv[1] == ATOM(xref)) {
			      ...
			      const ERL_NIF_TERM argv[])
{
	...
	if (enable && !vp->opt.xref) { // xref all clauses
		...
		for (i = 0; i < (int)vp->cnext[si]; i++)
		    clause_xref(vp, vp->clause_map[si][i]);
	    }
	}
	else if (!enable && vp->opt.xref) { // teardown xref
	    for (si = 0; si < NUM_CSET; si++) {	    
		for (i = 0; i < (int)vp->cnext[si]; i++)
		    clause_unxref(vp, vp->clause_map[si][i]);
	    }
	    vp->opt.xref = false;
	}
	return ATOM(ok);
				    ...
				    const ERL_NIF_TERM argv[])
{
	...
	dlit = (lit_t*) dmem_alloc(vp, size*sizeof(lit_t));	
	lpp = dlit;
...
static void remap_hash_slot(varp_t* vp,int i,unsigned int si,int* remap,int n)
{
    UNUSED(n);    
    hlink_t* hp = vp->hashtab[i];
    while (hp != NULL) {
