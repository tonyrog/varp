...
{
    unsigned k;
    enif_fprintf(stdout, "%s id=%d:%d,[%ld:%ld] [%s",
		 label, GET_SI(cp->cix), GET_IX(cp->cix),
		 cp->wl[0].p, cp->wl[1].p,
			      ...
			      const ERL_NIF_TERM argv[])
{
	...
	DBG_ORDER("%snbcp: utoggle decision=%s\r\n",
