...
#define TRUE    1
#define BOUND   2
#define NBOUND -2   // allow negation of BOUND (value)
...
typedef struct _varp_t {
    ...
    size_t       bound;       // number of bound variables
...
static inline int lit_negative(lit_t l)
{
#ifdef LIT_INTEGER
    return l < 0;
#else
...
static inline int lit_index(lit_t l)
{
#ifdef LIT_INTEGER
    return l;
#else
    return (l->sign < 0) ? -l->var->vix : l->var->vix;
#endif
}

static inline lit_t lit_negate(lit_t l)
{
#ifdef LIT_INTEGER
    return -l;
#else
    return (l->sign < 0) ? &l->var->lit[0] : &l->var->lit[1];
#endif
}

static inline literal_t* lit_literal(varp_t* vp, lit_t l)
{
#ifdef LIT_INTEGER
    return (l<0) ? &vp->var_map[-l]->lit[1] : &vp->var_map[l]->lit[0];
#else
    UNUSED(vp);
    return (literal_t*) l;
#endif
...
{
    literal_t* lit = lit_literal(vp,lp);
    return lit->var;
}
...
static inline int get_lv(varp_t* vp, literal_t* lp)
{
    return (lp->sign < 0) ? -get_vv(vp,lp->var) : get_vv(vp,lp->var);
}

// primitive set literal value
static inline void set_lv(varp_t* vp, literal_t* lp, int value)
{
    if (lp->sign < 0)
...
{
    return (var->bound) ? get_lv(vp, var->bound) : get_vv(vp, var);
}

static inline void set_variable_value(varp_t* vp, variable_t* var, int value)
{
    if (var->bound)
	set_lv(vp, var->bound, value);
    else
	set_vv(vp, var, value);
}
...
{
    if (lp->sign < 0)
	return -get_variable_value(vp,lp->var);
    else
	return get_variable_value(vp,lp->var);
}
...
{
    literal_t* lp = lit_literal(vp, l);
    return literal_value(vp, lp);
}
...
char* literal_format(varp_t* vp, literal_t* lp)
{
    static char varname[32];    
    UNUSED(vp);
    char* n = (lp->sign < 0) ? "!" : "";
...
char* lit_format(varp_t* vp, lit_t l)
{
    return literal_format(vp, lit_literal(vp, l));
}
...
static void lqueue_put(varp_t* vp, lit_t lp)
{
    ...
    assert(!is_constant(lit_value(vp, lp)));
...
static void push_variable(varp_t* vp, variable_t* var, int level)
{
    ...
    var->uvalue = get_variable_value(vp, var);
...
// return -1 otherwise
int clause_find(varp_t* vp, uint16_t op, lit_t* lit, size_t size)
{
    int i;
	...
	if ((cp->size == size) && (cp->op == op) && clause_is_equal(lit, cp->lit, size))
	    return cp->cix;
    }
    return -1;
}
					  ...
					  const ERL_NIF_TERM argv[])
{
	...
	if (!(lp->var->flags & VAR_FLAG_MARK)) { // not moved
	    ...
	    lp->var->map_index = mi;
	    map[mi++] = (lp->sign < 0) ? -lp->var->vix : lp->var->vix;
	}
	list = tail;
    }
			     ...
			     const ERL_NIF_TERM argv[])
{
    ...
    if (!enif_get_resource(env, argv[0], varp_res, (void**) &vp))
	return enif_make_badarg(env);
    if (!enif_get_int(env, argv[1], &lit))
	return enif_make_badarg(env);
				...
				const ERL_NIF_TERM argv[])
{
    ...
    if (!get_lit(env, vp, argv[1], &xp))
	return enif_make_badarg(env);
    return make_boolean(env, !is_constant(lit_value(vp, xp)));
}
...
static int watch_clause(varp_t* vp, clause_t* cp)
{
	...
	insert_sort3(lit_index(lp),lev,p,va,la,pa);
    }
    ...
    // setup watch
    set_wlink(&cp->wl[0], pa[1], lit_literal(vp, cp->lit[pa[1]]));
    set_wlink(&cp->wl[1], pa[0], lit_literal(vp, cp->lit[pa[0]]));
...
{
    literal_t* xp = lit_literal(vp, xl);
    literal_t* yp = lit_literal(vp, yl);
    variable_t* x = xp->var;
    variable_t* y = yp->var;
    xref_t** xpp  = &x->xfirst;
    ...
    set_vv(vp, y, BOUND);
    if (lit_negative(yl))
	y->bound = lit_literal(vp,lit_negate(xl));
    else
	y->bound = xp;
    vp->bound++;    
}
			     ...
			     const ERL_NIF_TERM argv[])
{
    ...
    if (ABS(lit_index(xp)) != ABS(lit_index(yp)))
	subst(vp, xp, yp);
    return ATOM(true);
}
			      ...
			      const ERL_NIF_TERM argv[])
{
	...
	return enif_make_int(env, vp->bound);
    }
    else if (argv[1] == ATOM(number_of_unbound_variables)) {
	return enif_make_int(env, vp->vnum - vp->bound);
    }
...
{
    variable_t* var = lit_variable(vp, cp->lit[p]);
				     ...
				     lit_t* lit, size_t size)
{
    ...
    if ((cix = clause_find(vp, op, lit, size)) >= 0) {
