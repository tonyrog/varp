...
rv(I) ->
    Ri = r(I),
    varp_formula:const_vector(uint,Ri,bit:size(Ri)).
...
eval(Data) ->
    Options = [{clause,true},{bcp,true},{backtrack,false}],
    Bs0 = varp_formula:new(Options),
    ...
    Cv = {p,'C',[]},
    Dv = {p,'D',[]},
    {_,Bs3} = varp_formula:operation(':=', Av, A, Bs2),
    {_,Bs4} = varp_formula:operation(':=', Bv, B, Bs3),
    {_,Bs5} = varp_formula:operation(':=', Cv, C, Bs4),
    {_,Bs6} = varp_formula:operation(':=', Dv, D, Bs5),
    try varp_formula:eval(Bs6) of
	false ->
	    false;
	true ->
	    Model = varp_formula:model(Bs6),
	    Am = proplists:get_value(Av, Model),
	    Bm = proplists:get_value(Bv, Model),
	    Cm = proplists:get_value(Cv, Model),
	    Dm = proplists:get_value(Dv, Model),
...
solve(Data, Spec) ->
    Order = ['-occure'], %% ++ [{uint,s,8,I} || I <- [0,1,2,3,4,5,6,7]],
    Opts = [{order,Order},
	    {clause,true},
	    {bcp,true},
	    {value, true},
    ...
    {Vs,Bs00} = bit_vector(Spec, varp_formula:new(Opts)),
    {Ws,Bs01} = group32_list(w,16,Vs,Bs00),
    %% {Ws,Bs0} = var32_list(w,16,formula:new(),[]),
    {{A,B,C,D},Bs1} = md5_block(list_to_tuple(Ws), Bs01),
    {C0,Cs0} = varp_formula:uint32(Ai,Bs1),
    {C1,Cs1} = varp_formula:uint32(Bi,Cs0),
    {C2,Cs2} = varp_formula:uint32(Ci,Cs1),
    {C3,Cs3} = varp_formula:uint32(Di,Cs2),
    {X1,Bs2} = varp_formula:operation('==',A,C0,Cs3),
    {X2,Bs3} = varp_formula:operation('==',B,C1,Bs2),
    {X3,Bs4} = varp_formula:operation('==',C,C2,Bs3),
    {X4,Bs5} = varp_formula:operation('==',D,C3,Bs4),
    {F,Bs6} = varp_formula:all([X1,X2,X3,X4], Bs5),
    %% Order = [occure_depth],
    varp_prover:run(F, Bs6).
...
group32_list(W, N, Vs, Bs, Gs) when is_atom(W) ->
    ...
    G = {uint,32,Xs},
    Bs1 = varp_formula:alias_vector(uint,W,32,Xs,Bs),
...
bit_vector([true|Vs], Bs, Acc) ->
    {{bool,T},Bs1} = varp_formula:build_(true, Bs),
    bit_vector(Vs, Bs1, [[T]|Acc]);
bit_vector([false|Vs], Bs, Acc) ->
    {{bool,F},Bs1} = varp_formula:build_(false, Bs),
...
bit_vector([{N,V}|Vs], Bs, Acc) when is_integer(N), N>0, is_atom(V) ->
    {{_Type,N,Xs},Bs1} = varp_formula:var_vector(uint,V,N,Bs),
    bit_vector(Vs, Bs1, [reverse(Xs)|Acc]);
bit_vector([N|Vs], Bs, Acc) when is_integer(N) ->
    {V,Bs1} = lists:foldl(
		fun(_I, {Vs0,Bs0}) ->
			{V,Bs1} = varp_formula:fresh_var(Bs0),
			{[V|Vs0],Bs1}
...
var32(V,I,Bs) when is_atom(V), is_integer(I) ->
    ...
    varp_formula:uint32(W,Bs).

uint32_list([V|Vs],Bs,Acc) ->
    {W,Bs1} = varp_formula:uint32(V,Bs),
...
md5_block(W,Bs0) ->
    {A0,Bs1} = varp_formula:uint32(16#67452301,Bs0),
    {B0,Bs2} = varp_formula:uint32(16#efcdab89,Bs1),
    {C0,Bs3} = varp_formula:uint32(16#98badcfe,Bs2),
    {D0,Bs4} = varp_formula:uint32(16#10325476,Bs3),
    {{A1,B1,C1,D1},Bs5} = md5_(0,W,A0,B0,C0,D0,Bs4),
    {A2,Bs6} = varp_formula:operation('+',A0,A1,Bs5),
    {B2,Bs7} = varp_formula:operation('+',B0,B1,Bs6),
    {C2,Bs8} = varp_formula:operation('+',C0,C1,Bs7),
...
md5_(I,W,A,B,C,D,Bs) when I < 16 ->
    ...
    {X1,Bs2} = varp_formula:operation('&',B1,D,Bs1),
    {X2,Bs3} = varp_formula:operation('&',B,C,Bs2),
    {F,Bs4} = varp_formula:operation('|',X1,X2,Bs3),
    md5t_(I,I,W,A,B,C,D,F,Bs4);

md5_(I, W, A, B, C, D,Bs) when I < 32 ->
    %% F = {'|',{'&',D,B},{'&',{'~',D},C}},
    {D1,Bs1} = varp_formula:operation('~',D,Bs),
    {X1,Bs2} = varp_formula:operation('&',D1,C,Bs1),
    {X2,Bs3} = varp_formula:operation('&',D,B,Bs2),
    {F,Bs4} = varp_formula:operation('|',X1,X2,Bs3),
    md5t_(I,(5*I+1) rem 16,W,A,B,C,D,F,Bs4);

md5_(I, W, A, B, C, D, Bs) when I < 48 ->
    %% F = {'^', {'^',B,C}, D},
    {X1,Bs1} = varp_formula:operation('^',B,C,Bs),
    {F,Bs2} = varp_formula:operation('^',X1,D,Bs1),
    md5t_(I,(3*I+5) rem 16,W,A,B,C,D,F,Bs2);

md5_(I, W, A, B, C, D, Bs) when I < 64 ->
    %% F = {'^', C, {'|',B, {'~', D}}},
    {D1,Bs1} = varp_formula:operation('~',D,Bs),
    {X1,Bs2} = varp_formula:operation('|',B,D1,Bs1),
    {F,Bs3} = varp_formula:operation('^',C,X1,Bs2),
...
md5t_(I, G, W, A, B, C, D, F, Bs) ->
    ...
    {X3,Bs5} = varp_formula:operation('+',A,F,Bs),
    {Ki,Ks}  = varp_formula:uint32(k(I),Bs5),
    {X4,Bs6} = varp_formula:operation('+',X3,Ki,Ks),
    {S,Bs7}  = varp_formula:operation('+',X4,element(G+1,W),Bs6),
    {R,Bs8}  = varp_formula:operation('<<<', S,rv(I),Bs7),
    {B2,Bs9} = varp_formula:operation('+',B,R,Bs8),
